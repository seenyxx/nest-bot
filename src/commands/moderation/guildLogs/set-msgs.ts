...
export default createSubCommand({
  ...
  requiredPermissions: PERMISSION_LEVELS.user.concat(['MANAGE_WEBHOOKS', 'MANAGE_CHANNELS']),
  category: 'moderation',
  guildOnly: true
}, async (msg, args) => {
  if (!msg.guild) return
  ...
  if (!me) return
  if (channel.type !== 'text' && channel.type !== 'news') throw new Error('The channel must a text channel.')
  if (channel.guild.id !== msg.guild.id) throw new Error('That channel is not in this server.')
  ...
  msg.reply(new Success('Updated Logging Channel', `Set ${mentionChannel(channel.id)} as the logging channel for this server.`))
})